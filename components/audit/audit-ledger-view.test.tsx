...
import type { AuditEventsResponse, AuditIntegrityResponse } from "@/lib/audit-api";
...
function response(nextCursor: string | null = null): AuditEventsResponse {
  return {
    items: [{
      ...
      changedFields: ["role"],
      details: { afterRole: "INSTRUCTOR", beforeRole: "LEARNER", revision: 4 },
...
function verified(checkpoint = "new-signed-checkpoint"): AuditIntegrityResponse {
  return {
    checkpoint,
    complete: true,
    continuation: null,
...
function renderView(client = createClient()) {
  return render(
    <QueryClientProvider client={client}>
      <AuditLedgerView scope={{ kind: "CURRENT_TENANT" }} token="tenant-token" viewerScope={viewer} />
    </QueryClientProvider>,
  );
}
...
describe("AuditLedgerView", () => {
  it("renders only redacted typed evidence and applies first-page filters", async () => {
    ...
    expect(screen.getAllByText("Đổi vai trò thành viên").length).toBeGreaterThan(0);
    ...
    expect(screen.getByText("Checkpoint snapshot chưa xác minh — hãy kiểm tra chuỗi trước khi lưu")).toBeTruthy();
    expect(screen.queryByRole("option", { name: "Chấp nhận lời mời" })).toBeNull();
    ...
    await waitFor(() => expect(api.listEvents).toHaveBeenLastCalledWith(
      { token: "tenant-token" },
      { kind: "CURRENT_TENANT" },
  ...
  it("uses only the opaque cursor for the next immutable snapshot page", async () => {
    ...
    fireEvent.click(await screen.findByRole("button", { name: "Tải thêm sự kiện" }));

    await waitFor(() => expect(api.listEvents).toHaveBeenNthCalledWith(
      2,
      { token: "tenant-token" },
      { kind: "CURRENT_TENANT" },
  ...
  it("verifies with an external checkpoint and exposes the new anchor", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Dán checkpoint từ lần kiểm tra trước"), {
      target: { value: "external-checkpoint" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Kiểm tra chuỗi" }));

    expect(await screen.findByText("Phạm vi incremental từ checkpoint hợp lệ")).toBeTruthy();
    ...
    expect(screen.getByText("Checkpoint incremental đã xác minh — lưu ngoài Mongo")).toBeTruthy();
  });
  ...
  it("labels a paginated genesis verification separately from an anchored range", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Kiểm tra chuỗi" }));
    expect(await screen.findByText("Phần đã kiểm tra hợp lệ; cần tiếp tục")).toBeTruthy();
    ...
    expect(screen.getByText("Checkpoint sau lần quét từ genesis — lưu ngoài Mongo")).toBeTruthy();
  });
  ...
  it("does not present the list snapshot checkpoint after integrity verification fails", async () => {
    ...
    expect(await screen.findByText(/Hash nội dung sự kiện không khớp/)).toBeTruthy();
  ...
  it("drops a late verification result after the authority scope changes", async () => {
    let resolveVerification: ((value: AuditIntegrityResponse) => void) | undefined;
    api.verifyIntegrity.mockReturnValue(new Promise((resolve) => { resolveVerification = resolve; }));
    ...
    rendered.rerender(
      <QueryClientProvider client={client}>
        <AuditLedgerView
          scope={{ kind: "PLATFORM_TENANT", tenantId: "507f1f77bcf86cd799439088" }}
          token="platform-token"
          viewerScope={{ membershipId: "platform", role: "SUPER_ADMIN", tenantId: "platform", viewerId: "root-user" }}
        />
      </QueryClientProvider>,
    );
