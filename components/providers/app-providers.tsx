...
function AuthProvider({
  ...
  const replaceSession = useCallback(
    async (payload: AuthResponse, generation: number) => {
      ...
      try {
        ...
        /* Keep the live session in memory. */
      }
      setSession(next);
      setLoading(false);
      rotateQueryClient();
  ...
  const clearSession = useCallback(() => {
    ...
    try {
      ...
      /* Session still clears in memory. */
    }
    ...
    const restore = async () => {
      ...
      try {
        ...
        const current = await apiFetch<
          Omit<AuthResponse, "accessToken"> & {
            workspaces?: WorkspaceSummary[];
          }
        >("/auth/me", { token: stored.token });
        if (
  ...
  useEffect(() => {
    ...
    try {
      ...
      try {
        ...
        /* Storage events remain as the cross-tab fallback. */
      }
      return;
    }
    const activeChannel = channel;
    return () => {
      if (authChannelRef.current === activeChannel)
  ...
  const refreshSession = useCallback(async () => {
    ...
    const generation = authGenerationRef.current;
    const current = await apiFetch<
      Omit<AuthResponse, "accessToken"> & { workspaces?: WorkspaceSummary[] }
    >("/auth/me", { token: requestedToken });
    if (
      generation !== authGenerationRef.current ||
      sessionTokenRef.current !== requestedToken
    )
      return;
  ...
  const consumeAuthResponse = useCallback(
    async (payload: AuthResponse, expectedGeneration: number) => {
  ...
  const updateEffectiveAccess = useCallback(
    (effectiveAccess: EffectiveAccess | null, tenantId: string) => {
      setSession((previous) => {
        if (previous.user?.tenantId !== tenantId) return previous;
        const next = {
          ...previous,
          effectiveAccess: normalizeEffectiveAccess(effectiveAccess),
        };
        try {
          localStorage.setItem(SESSION_KEY, JSON.stringify(next));
        } catch {
  ...
  const updateOrganization = useCallback((organization: Organization) => {
    setSession((previous) => {
      ...
      const next = {
        ...previous,
        organization:
          previous.user?.tenantId === organization._id
            ? organization
            : previous.organization,
        workspaces: previous.workspaces.map((workspace) =>
          workspace.tenantId === organization._id
            ? {
                ...workspace,
  ...
  const updateUserProfile = useCallback(
    (profile: Pick<CurrentUser, "sub" | "fullName" | "avatarUrl">) => {
      setSession((previous) => {
        if (!previous.user || previous.user.sub !== profile.sub)
          return previous;
        const next = {
          ...previous,
          user: {
  ...
  const theme = useMemo(
    () => ({
      ...
      token: {
        ...
        controlHeightLG: 44,
        fontFamily:
          '"Manrope Variable", Manrope, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',
        fontSize: 14,
...
export function useAuth() {
  ...
  if (!context)
    throw new Error("useAuth phải được dùng bên trong AppProviders");
  return context;
}
