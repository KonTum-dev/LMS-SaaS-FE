...
function Probe() {
  ...
  return <>
    ...
    <button onClick={() => queryClient.setQueryData(["private", "tenant-1"], { secret: true })} type="button">Seed cache</button>
    <button onClick={() => lateMutation.mutate()} type="button">Start late mutation</button>
    <button onClick={() => {
      const expectedGeneration = auth.captureAuthGeneration();
      void lateAuthResponse.promise
        .then((payload) => auth.consumeAuthResponse(payload, expectedGeneration))
        .then(() => lateAuthOutcome("accepted"))
        .catch(() => lateAuthOutcome("rejected"));
    }} type="button">Start late auth response</button>
    <button onClick={() => void auth.switchWorkspace("tenant-2").catch((caught) => setError(caught instanceof Error ? caught.message : "failed"))} type="button">Chuyển workspace</button>
    <button onClick={() => void auth.switchWorkspace("tenant-3").catch((caught) => setError(caught instanceof Error ? caught.message : "failed"))} type="button">Chuyển workspace 3</button>
    <button onClick={() => void auth.refreshSession()} type="button">Làm mới phiên</button>
    <button onClick={() => void auth.login("new@nova.test", "ValidPassword123").catch((caught) => setError(caught instanceof Error ? caught.message : "failed"))} type="button">Đăng nhập mới</button>
    <button onClick={() => {
      auth.updateOrganization({ ...organization, name: "Tên tenant cũ đến muộn" });
      auth.updateEffectiveAccess({ ...effectiveAccess, readOnly: true, state: "READ_ONLY" }, "tenant-1");
    }} type="button">Áp dụng phản hồi cũ</button>
    <button onClick={auth.logout} type="button">Đăng xuất</button>
  </>;
}

function renderRestoredSession(fetchImplementation?: typeof fetch) {
  localStorage.setItem("novalms-session", JSON.stringify({
    effectiveAccess: { ...effectiveAccess, readOnly: true, state: "READ_ONLY" },
    organization,
    token: "token-1",
    user,
  }));
  vi.stubGlobal("fetch", fetchImplementation ?? vi.fn().mockResolvedValue(new Response(
...
describe("AppProviders auth lifecycle", () => {
  it("thay access state cũ trong localStorage bằng dữ liệu mới từ auth/me", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("novalms-session") ?? "{}")).toMatchObject({
      effectiveAccess: { readOnly: false, state: "ACTIVE" },
    });
  });

  it("khôi phục được localStorage legacy không có workspaces", async () => {
    renderRestoredSession(vi.fn().mockResolvedValue(new Response(
  ...
  it("không để 401 auth/me cũ hủy response login mới hợp lệ", async () => {
    ...
    const switchedUser = { ...user, email: "new@nova.test", membershipId: "membership-2", tenantId: "tenant-2" };
    const switchedOrganization = { ...organization, _id: "tenant-2", name: "Lumen School", slug: "lumen" };
    ...
    staleRestore.resolve(new Response(
      JSON.stringify({ code: "UNAUTHORIZED", message: "Token cũ hết hạn" }),
      { headers: { "Content-Type": "application/json" }, status: 401 },
    ));
    freshLogin.resolve(new Response(
      JSON.stringify({ accessToken: "new-login-token", effectiveAccess, organization: switchedOrganization, user: switchedUser, workspaces }),
      { headers: { "Content-Type": "application/json" }, status: 200 },
    ));
  ...
  it("xoay QueryClient để mutation cũ đến muộn không thể bơm lại private cache sau logout", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Start late mutation" }));
    await waitFor(() => expect(retiredClient!.getMutationCache().getAll()[0]?.state.status).toBe("pending"));
    ...
    await waitFor(() => expect(retiredClient!.getQueryData(["late-private"])).toEqual({ secret: "late-tenant-data" }));
  ...
  it("từ chối AuthResponse đến muộn sau logout thay vì tái tạo phiên", async () => {
    renderRestoredSession();
    await screen.findByText("teacher@nova.test");
    fireEvent.click(screen.getByRole("button", { name: "Start late auth response" }));
    fireEvent.click(screen.getByRole("button", { name: "Đăng xuất" }));
    await screen.findByText("signed-out");

    act(() => lateAuthResponse.resolve(lateSessionPayload));
    await waitFor(() => expect(lateAuthOutcome).toHaveBeenCalledWith("rejected"));
  ...
  it("phát logout không kèm dữ liệu phiên sau khi xóa cache cục bộ", async () => {
    ...
    expect(broadcastChannels[0].postMessage).toHaveBeenCalledWith({ type: "LOGOUT" });
  });
  ...
  it("vẫn vận hành và logout cục bộ nếu trình duyệt từ chối BroadcastChannel", async () => {
    vi.stubGlobal("BroadcastChannel", class UnavailableBroadcastChannel {
      constructor() {
  ...
  it("đổi workspace thay toàn bộ phiên và xóa cache tenant cũ trước khi hiển thị phiên mới", async () => {
    const switchedOrganization = { ...organization, _id: "tenant-2", name: "Lumen School", slug: "lumen" };
    const switchedUser = { ...user, membershipId: "membership-2", role: "TENANT_ADMIN" as const, tenantId: "tenant-2" };
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(
        JSON.stringify({ accessToken: "token-2", effectiveAccess, organization: switchedOrganization, user: switchedUser, workspaces }),
        { headers: { "Content-Type": "application/json" }, status: 200 },
      ));
    renderRestoredSession(fetchMock as typeof fetch);
  ...
  it("auth/me đổi vai trò sẽ xóa cache thuộc quyền cũ trước khi cập nhật phiên", async () => {
    ...
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(
        JSON.stringify({ effectiveAccess, user: learnerUser, organization, workspaces }),
        { headers: { "Content-Type": "application/json" }, status: 200 },
      ));
    renderRestoredSession(fetchMock as typeof fetch);
  ...
  it("auth/me đổi membership trong cùng tenant sẽ xoay cache authority", async () => {
    ...
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(
        JSON.stringify({ effectiveAccess, user: reissuedMembership, organization, workspaces }),
        { headers: { "Content-Type": "application/json" }, status: 200 },
      ));
    renderRestoredSession(fetchMock as typeof fetch);
    await screen.findByText("teacher@nova.test");
  ...
  it("giữ nguyên token, tổ chức và cache khi đổi workspace thất bại", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("novalms-session") ?? "{}")).toMatchObject({
      organization: { _id: "tenant-1" }, token: "token-1",
    });
  });

  it("bỏ qua 401 đến muộn từ token cũ nhưng logout khi chính token hiện tại hết hạn", async () => {
    const switchedOrganization = { ...organization, _id: "tenant-2", name: "Lumen School", slug: "lumen" };
    const switchedUser = { ...user, membershipId: "membership-2", tenantId: "tenant-2" };
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(
        JSON.stringify({ effectiveAccess, user, organization, workspaces }),
        { headers: { "Content-Type": "application/json" }, status: 200 },
    ...
    window.dispatchEvent(new CustomEvent("auth:expired", { detail: { token: "token-1" } }));
    expect(screen.getByText("token:token-2")).toBeTruthy();

    window.dispatchEvent(new CustomEvent("auth:expired", { detail: { token: "token-2" } }));
    await screen.findByText("signed-out");
    expect(localStorage.getItem("novalms-session")).toBeNull();
  });
  ...
  it("không logout bởi 401 token cũ trong lúc đang hủy query trước khi commit token mới", async () => {
    ...
    vi.spyOn(exposedQueryClient!, "cancelQueries").mockReturnValue(cancellation.promise);
  ...
  it("không để auth/me dùng token cũ ghi đè workspace vừa chuyển", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /^Chuyển workspace$/ }));
    await screen.findByText("token:token-2");
    staleRefresh.resolve(new Response(
      JSON.stringify({ effectiveAccess: { ...effectiveAccess, readOnly: true, state: "READ_ONLY" }, user, organization, workspaces }),
      { headers: { "Content-Type": "application/json" }, status: 200 },
    ));
  ...
  it("hai yêu cầu chuyển đồng thời dùng quy tắc yêu cầu mới nhất thắng", async () => {
    ...
    const organizationThree = { ...organization, _id: "tenant-3", name: "Orion Center", slug: "orion" };
    const userThree = { ...user, membershipId: "membership-3", tenantId: "tenant-3" };
    const organizationTwo = { ...organization, _id: "tenant-2", name: "Lumen School", slug: "lumen" };
    const userTwo = { ...user, membershipId: "membership-2", tenantId: "tenant-2" };
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(
        JSON.stringify({ effectiveAccess, user, organization, workspaces }),
        { headers: { "Content-Type": "application/json" }, status: 200 },
      ...
      .mockResolvedValueOnce(new Response(
        JSON.stringify({ accessToken: "token-3", effectiveAccess, organization: organizationThree, user: userThree, workspaces }),
        { headers: { "Content-Type": "application/json" }, status: 200 },
      ));
    renderRestoredSession(fetchMock as typeof fetch);
    await screen.findByText("organization:tenant-1");
    ...
    firstSwitch.resolve(new Response(
      JSON.stringify({ accessToken: "token-2", effectiveAccess, organization: organizationTwo, user: userTwo, workspaces }),
      { headers: { "Content-Type": "application/json" }, status: 200 },
    ));
  ...
  it("bỏ qua organization và entitlement cũ sau khi đã đổi tenant", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Áp dụng phản hồi cũ" }));

    expect(screen.getByText("organization:tenant-2")).toBeTruthy();
    expect(screen.getByText("access:ACTIVE")).toBeTruthy();
    expect(screen.getByText("workspace-one:Tên tenant cũ đến muộn")).toBeTruthy();
  });
});
