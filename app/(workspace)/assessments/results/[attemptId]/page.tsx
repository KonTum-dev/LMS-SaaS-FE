...
function AssessmentResultView({ attemptId, scope, token }: { attemptId: string; scope: ViewerScope; token: string }) {
  ...
  if ((attemptQuery.isPending && !attemptQuery.data) || resultQuery.isPending) {
    return <div aria-label="Đang tải kết quả" className="page-loading" role="status"><Spin size="large" /></div>;
  }
      ...
      <Result
        extra={(
          <div className={styles.inlineActions}>
            ...
            {!hidden && <Button icon={<ReloadOutlined />} onClick={() => void resultQuery.refetch()} type="primary">Thử lại</Button>}
          </div>
        )}
        status={hidden ? "404" : "error"}
        subTitle={hidden ? "Kết quả không còn khả dụng trong workspace của bạn." : resultQuery.error instanceof Error ? resultQuery.error.message : "Không thể tải kết quả."}
        title={hidden ? "Không tìm thấy kết quả" : "Không tải được kết quả"}
      />
    );
  }
  ...
  if (result.status === "IN_PROGRESS") {
    return (
      <Result
        extra={<Button onClick={() => router.replace(`/assessments/attempts/${attemptId}`)} type="primary">Tiếp tục làm bài</Button>}
        status="info"
        subTitle="Bạn cần kết thúc lượt làm trước khi có trạng thái kết quả."
        title="Lượt làm vẫn đang mở"
      />
    );
  }
  if (!result.resultReleased || !result.result) {
    ...
    const pending = detail
      ? resultPendingMessage(detail.resultVisibility, detail.closesAt)
      : "Kết quả chưa đến thời điểm công bố theo chính sách của bài kiểm tra.";
    return (
      <main aria-labelledby="result-title" className="page-shell">
        <header className={`${styles.pageHeader} page-heading`}>
          <div className="page-heading-copy">
            ...
            <h1 id="result-title">Trạng thái kết quả</h1>
            <p>Lượt làm {result.attemptNumber}</p>
          </div>
        ...
        <Card className="surface-card">
          <Result
            extra={(
              <div className={styles.inlineActions}>
                <Button onClick={() => router.push("/assessments")}>Về danh sách</Button>
                <Button icon={<ReloadOutlined />} loading={resultQuery.isFetching} onClick={() => void Promise.all([resultQuery.refetch(), detailQuery.refetch()])} type="primary">Kiểm tra lại</Button>
              </div>
            )}
            status="info"
            subTitle={pending}
            title={result.status === "TIMED_OUT" ? "Lượt làm đã hết giờ" : "Bài đã được ghi nhận"}
          />
        </Card>
      </main>
    );
  ...
  return (
    <main aria-labelledby="result-title" className="page-shell">
      <header className={`${styles.pageHeader} page-heading`}>
        <div className="page-heading-copy">
          <Button icon={<ArrowLeftOutlined />} onClick={() => router.push("/assessments")} type="link">Bài kiểm tra</Button>
          <h1 id="result-title">Kết quả lượt làm {result.attemptNumber}</h1>
          <p>Kết quả tổng hợp do máy chủ chấm. V1 không công bố đáp án hoặc đúng/sai theo từng câu.</p>
        </div>
        <AttemptStatusTag status={result.status} />
      </header>
      <Card className="surface-card">
        <div className={styles.resultHero}>
          <Tag color={score.passed ? "green" : "red"}>{score.passed ? "Đạt" : "Chưa đạt"}</Tag>
          <div aria-label={`${score.percentage.toLocaleString("vi-VN")}%`} className={styles.score}>{score.percentage.toLocaleString("vi-VN")}%</div>
          <Progress aria-label="Tỷ lệ điểm" percent={Math.max(0, Math.min(100, score.percentage))} showInfo={false} status={score.passed ? "success" : "normal"} />
          <p><strong>{score.score.toLocaleString("vi-VN")}</strong> / {score.maxScore.toLocaleString("vi-VN")} điểm</p>
          <div className={styles.inlineActions}>
            {attempt?.assessmentId && <Button onClick={() => router.push(`/assessments/${attempt.assessmentId}`)}>Xem bài kiểm tra</Button>}
            <Button onClick={() => router.push("/assessments")} type="primary">Về danh sách</Button>
          </div>
        </div>
      </Card>
    </main>
...
export default function AssessmentResultPage() {
  ...
  if (user?.role !== "LEARNER") return <Alert showIcon title="Chỉ học viên được xem kết quả lượt làm của mình." type="error" />;
